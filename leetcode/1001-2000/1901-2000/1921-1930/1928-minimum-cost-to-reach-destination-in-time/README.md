...
2D integer array `edges` where `edges[i] = [x$_i$, y$_i$, time$_i$]` denotes a
road between cities `x$_i$` and `y$_i$` that takes `time$_i$` minutes to travel.
