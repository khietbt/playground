...
- There are `n` language numbered `1` through `n`.
- `languages[i]` is the set of languages the `i$^th$` user knows, and
- `friendships[i] = [u$_i$, v$_i$]` denotes a friendship between the user
  `u$_i$` and `v$_i$`.

You can choose **one** language and teach it to some users so that all friends
can communicate with each other. Return _the **minimum** number of users you
need to teach_.

Note that friendships are not transitive, meaning if `x` is a friend of `y`, and
`t` is a friend of `z`, this doesn't guarantee that `x` is a friend of `z`.
...
> **Output:** 1
>
> **Explanation:** You can teach user 1 the second language or user 2 the first
> language.
...
- All tuples `(u$_i$, v$_i$)` are unique
