...
In other words, if you can divide `nums` into the subarrays
`nums[0..(i1 -1)], nums[i1..(i2 -1)], ..., nums[i$_k-1$...n -1]`, then
`i$_k-1$ - i$_1$ <= dist`.
...
> `[1, 3]`, `[2, 6, 4]`, and `[2]`.
>
> This choice is valid because i$_k-1$ - i$_1$ is 5 - 2 = 3 which is equal to
> dist. The total cost is `nums[0] + nums[2] + nums[5] = 1 + 2 + 2 = 5`.
>
> It can be shown that there is no possible way to divide nums into 3 subarrays
...
> `[10]`, `[1]`, `[2]` and `[2, 2, 1]`.
>
> This choice is valid because i$_k-1$ - i$_1$ is 3 - 1 = 2 which is less than
> dist. The total cost is `nums[0] + nums[1] + nums[2] + nums[3]` which is
> `10 + 1 + 2 + 2 = 15`.
>
> The division `[10]`, `[1]`, `[2, 2, 2]` and `[1]` is not valid because the
> difference between i$_k-1$ and i$_i1$ is 5 - 1 = 4, which is greater than dist.
>
> It can be shown that there is no possible way to divide nums into 4 subarrays
> at a cost lower than 15.
...
> **Output:** 36
>
> **Explanation:** The best way to divide nums into 3 subarrays is: `[10]`, `[8]`
> and `[18, 9]`. This choice is valid because i$_k-1$ - i$_1$ = 2 - 1 = 1, which
> is equal to dist. The total cost is `nums[0] + nums[1] + nums[2]` which is
> 10 + 8 + 18 = 36.
>
> The division `[10]`, `[8, 18]`, and `[9]` is not valid, because the difference
> between i$_k-1$ and i$_i1$ is 3 - 1 = 2, which is greater than dist.
>
> It can be shown that there is no possible way to divide nums into 3 subarrays
> at a cost lower than 36.
