...
For every contiguous `k x k` **submatrix** of `grid`, compute thhe **minimum
absolute** difference between two **distinct** values within that **submatrix**

Return a 2D array `answer` of size `(m - k + 1) x (n - k + 1)`, where `ans[i][j]`
...
A submatrix `(x1, y1, x2, y2)` is a matrix that formed by choosing all cells
...
>     - The minimum absolute difference in the submatrix `|3 - 5| = 2`.
