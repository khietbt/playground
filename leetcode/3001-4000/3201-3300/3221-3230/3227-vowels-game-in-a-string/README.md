...
Return `ture` if Alice wins the game, and `false` otherwise.
...
>   `s = "**leetco**der"` which contains 3 vowels. The resulting string is
>   `s = "der"`.
> - Bob plays second, he can delete underlined substring `s = "**d**er` which
>   contains 0 vowels. The resulting string is `s = "er"`.
> - Alice plays third, she can delete the who string `s = "er"` which contains 1
>   vowel.
> - Bob plays forth, since the string is empty, there is no valid play for Bob.
