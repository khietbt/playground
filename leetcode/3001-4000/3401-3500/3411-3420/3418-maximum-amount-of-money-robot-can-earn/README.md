...
`(0, 0)` and wants to reach the bottom-right `(m - 1, n - 1)`. The robot can
...
> 3. Move to `(1, 1)`, where there's a robber stealing `2` coins. The robot uses
...
> 3. Move to `(0, 2)`, gaining `10` coins (total coins = `20 + 10 = 30`).
> 4. Move to `(1, 2)`, gaining `10` coins (total coins = `30 + 10 = 40`).
