...
You are given a **0-indexed** strings `source` and `target`, both of length `n`
...
**substring** `x` from the string, and change it to `y` at a cost `z` **if**
...
- The substrings picked in the operation are `source[a..b]` and `source[c..d]`
...
Return _the **minimum** cost to convert the string `source` to `target` using
**any** number of operations. If it is impossible to convert `source` to
`target`, return `-1`_.
...
- `1 <= sources.length == target.length <= 1000`
