...
**second** row has `2` glasses, and so on until the 100th row. Each glass holds
...
half, and the two outside glasses are a quarter full, as pictured below.

![0799-1](./0799-1.png)

Now after pouring some non-negative integer cups of champagne, return how full
the `j-th` glass in `i-th` row is (both `i` & `j` are 0-indexed).
...
> indexed as (1, 0) and the glass indexed as (1, 1) will be share the excess
