...
Given the heads of two singly linked-lists `headA` and `headB`, return _the node
at which the two lists intersect_. If the two linked lists have no intersection
at all, return `null`.
...
>   1 in A and B (2nd node in A and 3rd in B) are different node references. In
...
> 0 if the two lists intersect).
>
> From the head of A, it reads are `[1, 9, 1, 2, 4]`. From the head of B, it
...
> must be 0, while skipB can be arbitrary values.
>
...
- `intersectVal == list[skipA] == listB[skipB]` if `listA` and `listB` intersect.
  **Follow up:** Could you write a solution that runs in `O(m + n)` time use only
  `O(1)` memory?
